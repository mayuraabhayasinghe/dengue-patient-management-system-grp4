  ...
  useEffect(() => {
    const fetchPatients = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:5000/api/patients/registered', {
          headers: {
    ...
    <motion.div
      ...
      <motion.div
          ...
          <div className="bg-white shadow-md rounded-lg p-4 border-l-4 border-yellow-500">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{patientCounts.monitoring}</p>
              </div>
              <FontAwesomeIcon icon={faHeartbeat} className="text-yellow-500 text-xl" />
            </div>
          </div>
        ...
        <div className="bg-white shadow-lg rounded-lg overflow-hidden">
          {patients.length === 0 ? (
            <div className="p-6 text-center text-gray-500">No registered patients found</div>
          ) : (
